 ...
 */
function the_wp_business_logo_customize_register( $wp_customize ) {
	...
	$wp_customize->add_control( 'logo_width', array(
		'label'       => esc_html__( 'Logo Resizer','the-wp-business' ),
		'description'       => esc_html__( 'Forces the logo to have a specified width(max value is 100).','the-wp-business' ),
...
add_action( 'customize_register', 'the_wp_business_logo_customize_register' );

/**
 * Add support for logo resizing by filtering `get_custom_logo`
 */
function the_wp_business_customize_logo_resize( $html ) {
	...
	if ( is_numeric( $size ) && is_numeric( $custom_logo_id ) ) {
		...
		if ( $logo['width'] >= $logo['height'] ) {
			$output = the_wp_business_min_max( $logo['height'], $logo['width'], $max['height'], $max['width'], $size, $min );
			$img = array(
				'height'	=> $output['short'],
				'width'		=> $output['long']
			);
		// portrait
			...
			$output = the_wp_business_min_max( $logo['width'], $logo['height'], $max['width'], $max['height'], $size, $min );
			$img = array(
				'height'	=> $output['long'],
				'width'		=> $output['short']
			);
...
add_filter( 'get_custom_logo', 'the_wp_business_customize_logo_resize' );

/* Helper function to determine the max size of the logo */
function the_wp_business_min_max( $short, $long, $short_max, $long_max, $percent, $min ){
	$ratio = ( $long / $short );
...
*/
function the_wp_business_customize_controls_js() {
	wp_enqueue_script( 'the-wp-business-customizer-controls', get_template_directory_uri() . '/inc/logo/js/customize-controls.js', array( 'jquery', 'customize-preview' ), '201709071000', true );
}
add_action( 'customize_controls_enqueue_scripts', 'the_wp_business_customize_controls_js' );

/**
 * Adds CSS to the Customizer controls.
 */
function the_wp_business_customize_css() {
	wp_add_inline_style( 'customize-controls', '#customize-control-logo_size input[type=range] { width: 100%; }' );
}
add_action( 'customize_controls_enqueue_scripts', 'the_wp_business_customize_css' );

/**
 * Testing function to remove logo_width theme mod
 */
function the_wp_business_remove_theme_mod() {
	if ( isset( $_GET['remove_logo_size'] ) && 'true' == $_GET['remove_logo_size'] ){
		set_theme_mod( 'logo_width', '' );
	}
}
add_action( 'wp_loaded', 'the_wp_business_remove_theme_mod' );