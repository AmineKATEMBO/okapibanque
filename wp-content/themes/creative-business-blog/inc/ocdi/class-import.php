...
if ( ! class_exists( 'Creative_Business_Blog_About' ) ) {
	 ...
	 * @since 1.0.0
	 */
	class Creative_Business_Blog_About {
		 ...
		 * Page configuration.
		 *
		 * @var array $importconfig Configuration.
		 */
		...
		/**
		 * Singleton instance of Creative_Business_Blog_About.
		 *
		 * @var Creative_Business_Blog_About $instance Creative_Business_Blog_About instance.
		 */
		private static $instance;

		/**
		 * Main Creative_Business_Blog_About instance.
		 *
		 * @since 1.0.0
		 *
		 * @param array $importconfig Configuration array.
		 */
		public static function init( $importconfig ) {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Creative_Business_Blog_About ) ) {
				self::$instance = new Creative_Business_Blog_About;
				if ( ! empty( $importconfig ) && is_array( $importconfig ) ) {
					self::$instance->config = $importconfig;
		...
		public function setup_config() {
			...
			$this->logo_url      = isset( $this->config['logo_url'] ) ? $this->config['logo_url'] : get_template_directory_uri() . '/inc/ocdi/images/logopostmag.png';
			$this->logo_link     = isset( $this->config['logo_link'] ) ? $this->config['logo_link'] : 'https://www.postmagthemes.com/';
			$this->tabs          = isset( $this->config['tabs'] ) ? $this->config['tabs'] : array();
			/* translators: %1$s is replaced with "string" */
		 /* translators: %2$s is replaced with "string" */
		 /* translators: %3$s is replaced with "string" */
		 /* translators: %s is replaced with "string" */
		/* translators: %s: get started */
			$this->notification  = isset( $this->config['notification'] ) ? $this->config['notification'] : ( '<p>' . sprintf( esc_html__( 'Welcome! Thank you for choosing %1$s! To fully take advantage of the best our theme can offer please make sure you visit our %2$swelcome page%3$s.', 'creative-business-blog' ), $this->theme_name, '<a href="' . esc_url( admin_url( 'themes.php?page=' . $this->page_slug ) ) . '">', '</a>' ) . '</p><p><a href="' . esc_url( admin_url( 'themes.php?page=' . $this->page_slug ) ) . '" class="button button-primary" style="text-decoration: none;">' . sprintf( esc_html__( 'Get started with %s', 'creative-business-blog' ), $this->theme_name ) . '</a></p>' );
		...
		public function render_about_page() {
			...
			if ( ! empty( $welcome_title ) || ! empty( $welcome_content ) || ! empty( $this->tabs ) ) {
				...
				// Display tabs.
				if ( ! empty( $this->tabs ) ) {
					$active_tab = isset( $_GET['tab'] ) ? sanitize_text_field( wp_unslash( $_GET['tab'] )) : 'recommended_actions';
		...
		public function load_assets( $hook ) {
			...
			if ( 'appearance_page_' . $this->page_slug === $hook ) {
				...
				wp_enqueue_style( 'creative-business-blog', get_template_directory_uri() . '/inc/ocdi/css/about.css', array(), '1.0.0' );				
			}
		}
						...
						if ( ! empty( $action_value['plugin_slug'] ) ) {
							...
							switch ( $active['needs'] ) {
								...
								case 'install':
									...
									$label = esc_html__( 'Install', 'creative-business-blog' );
									break;
								case 'activate':
									$class = 'activate-now button button-primary';
									$label = esc_html__( 'Activate', 'creative-business-blog' );
									break;
								case 'deactivate':
									$class = 'deactivate-now button';
									$label = esc_html__( 'Deactivate', 'creative-business-blog' );
									break;
							}
							?>
		...
		public function dismiss_recommended_action_callback() {
			...
			$todo      = ( isset( $_GET['todo'] ) ) ? esc_attr( sanitize_text_field( wp_unslash( $_GET['todo'] ) )): '';
			$action_id = ( isset( $_GET['id'] ) ) ? esc_attr( sanitize_text_field( wp_unslash( $_GET['id'] ) ) ): '';
			$wpnonce   = ( isset( $_GET['_wpnonce'] ) ) ? esc_attr( sanitize_text_field( wp_unslash( $_GET['_wpnonce'] ) ) ) : '';
		...
		public function get_plugins( $plugin_folder = '' ) {
			if ( ! function_exists( 'get_plugins' ) ) {
				get_template_part('wp-admin/includes/plugin', 'creative-business-blog');
				// require_once ABSPATH . 'wp-admin/includes/plugin.php' has been used before
			}
		...
		private function is_plugin_active( $slug ) {
			...
			get_template_part('wp-admin/includes/plugin', 'creative-business-blog');
				//require_once ABSPATH . 'wp-admin/includes/plugin.php' this can be used with absolute path
			}
